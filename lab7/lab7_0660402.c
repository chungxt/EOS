...
void handler(int signo, siginfo_t *info, void *context){
	...
	if((log_shmid = shmget(log_key, bufSize, 0666)) < 0){
		perror("shmget");
		exit(1);
	}
	if((log_shm = shmat(log_shmid, NULL, 0)) == (char *) -1){
		perror("shmat");
		exit(1);
	}
	

	//printf("process (%d) get signal from process (%d)\n", getpid(), info->si_pid);
	//printf(" => %s\n", log_shm);
	write(1, log_shm, strlen(log_shm));
	*log_shm = '\0';	
...
void timer_handler(int signum)
{
	...
	if((timer_shmid = shmget(timer_key, bufSize, 0666)) < 0){
		perror("shmget");
		exit(1);
	}
	if((timer_shm = shmat(timer_shmid, NULL, 0)) == (char *) -1){
		perror("shmat");
		exit(1);
	}
...
void *threadUberDriving(void *threadid)
{
	...
	int timer_shmid, log_shmid;
	key_t timer_key, log_key;
	char *timer_shm, *log_shm;
	
	struct timeval start;
	struct timeval end;
	float diff;

	// because use sleep(), we can't use clock_t, then I use gettimeofday
	...
	*/
	...
	if(sX != eX && sY != eY){
		sprintf(str, "(%d, %d)-->(%d, %d): %d/%f [(%d, %d)-%s->(%d, %d)-->(%d, %d)]\n", sX, sY, eX, eY, distance, diff, sX, sY, str2, eX, sY, eX, eY);
	}
	else if (sX != eX || sY != eY){
		sprintf(str, "(%d, %d)-->(%d, %d): %d/%f [(%d, %d)-%s->(%d, %d)]\n", sX, sY, eX, eY, distance, diff, sX, sY, str2, eX, eY);
	}
	else {
		sprintf(str, "(%d, %d)-->(%d, %d): %d/%f [(%d, %d)-%s-]\n", sX, sY, eX, eY, distance, diff, eX, eY, str2);
	}
		
	
	// printf("client write path info to the share memory\n");
	strcat(timer_shm, str);
	strcat(log_shm, str);
	...
	//printf("%s", str);
	kill(upid, SIGUSR1);
	printf("process (%d) send signal to process (%d)X (%d)O\n", getpid(), getppid(), upid);
...
void ParentProcess()
{
	...
	if((timer_shmid = shmget(timer_key, bufSize, IPC_CREAT | 0666)) < 0){
		perror("shmget timer");
		exit(1);
	}
	if((timer_shm = shmat(timer_shmid, NULL, 0)) == (char *) -1){
	...
	if((log_shmid = shmget(log_key, bufSize, IPC_CREAT | 0666)) < 0){
		perror("shmget log");
		exit(1);
	}
	if((log_shm = shmat(log_shmid, NULL, 0)) == (char *) -1){
	...
	struct sigaction log_sa, timer_sa;
	struct itimerval timer;
	
	memset(&log_sa, 0, sizeof(struct sigaction));
	log_sa.sa_flags = SA_SIGINFO;
	log_sa.sa_sigaction = handler;
	
	sigaction(SIGUSR1, &log_sa, NULL);
	...
	retval = shmctl(timer_shmid, IPC_RMID, NULL);
	if(retval < 0){
		fprintf(stderr, "server remove share memory failed\n");
		exit(1);
	}

	retval = shmctl(log_shmid, IPC_RMID, NULL);
	if(retval < 0){
		fprintf(stderr, "server remove share memory failed\n");
		exit(1);
	}
