...
void enqueue_Node(LinkedList *list ,int pri, int rank, const char* tsk) { // include sort & enqueue
...
void dequeue_Node( LinkedList* list, char str[]) { // pop order by taskPriority
	...
	if (now != NULL){
		sprintf(str, "===== pop the highest task => task name: %s, task priority: %d, task rank: %d =====\n", now->taskName, now->taskPriority, now->taskRank);
		printf("%s", str);		
...
void print_LinkedList (LinkedList *list, int str[]) { // delete a linked list (free all nodes)
	ListNode *now;
	int count = 0;
	now = list->head;
	while (now != NULL){
		printf("%d:\ttask name: %s,\ttask priority: %d,\ttask rank: %d\n", count, now->taskName, now->taskPriority, now->taskRank);	
		str[count] = now->taskPriority;
		now = now->successor;
		count++;
	}
